...
import (
	...
	"github.com/sudachen/pprof/internal/graph"
...
func makeWebInterface(p *profile.Profile, opt *Options) *webInterface {
...
func serveWebInterface(hostport string, p *profile.Profile, o *Options) error {
	...
	args := &plugin.HTTPServerArgs{
		...
		Handlers: map[string]http.Handler{
			...
			"/clear":      http.HandlerFunc(ui.clear),
			"/":           http.HandlerFunc(ui.update),
		},
	}

	if o.WantBrowser {
		go openBrowser("http://"+args.Hostport, &o.internal)
	}
	return server(args)
}
...
func (ui *webInterface) makeReport(w http.ResponseWriter, req *http.Request,
	...
	options := *ui.options
	options.UI = catcher
	_, rpt, err := generateRawReport(ui.prof, cmd, v, &options.internal)
	if err != nil {
...
func (ui *webInterface) disasm(w http.ResponseWriter, req *http.Request) {
	...
	out := &bytes.Buffer{}
	if err := report.PrintAssembly(out, rpt, ui.options.internal.Obj, maxEntries); err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		ui.options.UI.PrintErr(err)
		return
	}
...
func (ui *webInterface) source(w http.ResponseWriter, req *http.Request) {
	...
	if err := report.PrintWebList(&body, rpt, ui.options.internal.Obj, maxEntries); err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		ui.options.UI.PrintErr(err)
		return
	}
...
func (ui *webInterface) peek(w http.ResponseWriter, req *http.Request) {
	...
	out := &bytes.Buffer{}
	if err := report.Generate(out, rpt, ui.options.internal.Obj); err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		ui.options.UI.PrintErr(err)
		return
	}
...
func (ui *webInterface) update(w http.ResponseWriter, req *http.Request) {
	...
	url := "/dot"
	if ui.options.Fetch != nil {
		p, _, err = ui.options.Fetch.Fetch("", 0, 0)
	}
	if err != nil {
...
func (ui *webInterface) clear(w http.ResponseWriter, req *http.Request) {
	rpt, errList := ui.makeReport(w, req, []string{"top"}, "nodecount", "1")
	if rpt == nil {
		return // error already reported
	}
