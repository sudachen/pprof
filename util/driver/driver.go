...
func PProf(eo *Options) error {
	// Remove any temporary files created during pprof processing.
	defer cleanupTempFiles()
	o := setDefaults(eo)

	src, cmd, err := parseFlags(&o.internal)

	if err != nil {
		return err
	}

	p, err := fetchProfiles(src, &o.internal)
	if err != nil {
		return err
	}

	if cmd != nil {
		return generateReport(p, cmd, pprofVariables, &o.internal)
	}
	...
	return interactive(p, &o.internal)
}
